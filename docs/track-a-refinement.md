...
The refactor landed and was measured against three real Wikipedia ZIMs spanning roughly 100× in size. Heap moves from 20 MB to 56 MB across that spread, and what it tracks is dirent count rather than bytes — the 24 MB archive uses more heap than the 318 MB one because it holds six times as many entries.

| **Archive** | **Size** | **Dirents** | **Wall** | **Heap** | **Working set** |
|---|---|---|---|---|---|
| en_chemistry_mini | 24 MB | 58,260 | 4.6 s | 23.5 MB | 36 MB |
| en_100 | 318 MB | 9,337 | 0.9 s | 20.5 MB | 67 MB |
| en_history_maxi | 2,267 MB | 382,612 | 63 s | 55.9 MB | 2,150 MB |

- **The working-set column is not memory pressure, and the proof is not an argument:** It scales with input because the ZIM is memory-mapped and every blob read touches a mapped page, which Windows charges to the working set — clean, file-backed, reclaimable pages that no commit limit counts and no OOM killer considers (the same split as RssFile versus RssAnon on Linux). Rather than reason about it, the 2,267 MB conversion was run inside a Windows Job Object with a hard 128 MB commit limit: it completed, the kernel reported a peak commit of 57.1 MB, and the 2.4 GB result verified all 382,608 entries. A naive ps reading of this process would show 2 GB and be wrong.
- **[Moderate]** The brief for this pass called zim2wax's pre-pass "metadata-only, which is cheap" — true per byte and false per entry. An in-memory url-to-index map cost about 210 bytes per dirent: 80 MB at 382k dirents, and roughly 1.3 GB extrapolated to full Wikipedia's 6.5M, which does not fit the 512 MB floor. Resolved by moving the map into a temporary SQLite file. The generalizable rule, which this document had not stated: bounded per byte is not bounded per entry, and at six million entries the per-entry constant is the thing that decides whether a pass fits. Residual per-dirent state is now about 20 bytes, roughly 130 MB at full Wikipedia — inside the floor, but a quarter of it, so it is a known ceiling rather than a solved problem.
- **Conversion is a publishing-side operation and never a device-side one:** Extrapolating the temporary index to full Wikipedia gives roughly 1.7 GB of scratch on disk — not RAM, but it has to exist somewhere, and a 100 GB output plainly does not belong on a box whose Kiosk-tier storage floor is 16 GB. Conversion therefore runs where B13 publishes, on Community Hub-class hardware or better, exactly as §5 already scopes delta computation. A device consumes packs; it does not build them. Stated here because the streaming work makes conversion look device-viable on memory grounds alone, and it is not.
- **Two ceilings deliberately left standing:** open_append still validates redirect targets by reading prior segments through the full reader, which is O(existing entries) in memory. No converter uses append and nothing at scale exercises it; the streaming variant would attach the prior segment databases instead. Separately, the 100 GB case remains extrapolated rather than measured — the largest real archive obtainable was 2.27 GB, and the flat-heap trend plus the commit-limited run are strong evidence, not a run.

### The next ceiling is the reader, and it is the more serious one

This pass fixed the write side. The read side has the same defect and worse consequences: WaxReader::open merges every entry into an in-memory map, so opening a pack costs memory proportional to its entry count. At full Wikipedia's roughly 6.5 million entries that is several hundred megabytes to open a file, on a tier whose floor is 512 MB total — meaning the pack a Kiosk box exists to serve is the pack it cannot open.

- **Why this is not merely A2e's mirror image:** A conversion that will not fit can be run elsewhere; that is what the publishing-side scoping above settles. A pack that will not open has nowhere else to go — serving it on constrained hardware is the product. This sits directly in A5's path (wax-serve, P1) and should be designed before A5 rather than discovered inside it.
- **The format already answers it, the same way it answered the writer:** The index is SQLite. A reader that queries it — point lookups by path, and ATTACH across the segment chain for last-segment-wins — holds no entry table at all. The in-memory map exists because merging segments in Rust was the obvious first implementation, not because the format requires it.
